...
>
  <div class="flex items-center px-4">
    <!-- Tombol Settings (FIXED - di luar kontainer scroll) -->
    <div class="flex-shrink-0">
      <button
        @click="openSettingsModal"
        class="p-2 rounded-full text-gray-600 hover:text-gray-800 hover:bg-gray-100 transition-colors duration-200"
        title="Pengaturan"
      >
    ...
    <!-- Kontainer scrollable untuk menu items -->
    <div
      ref="menuContainer"
      class="flex-1 overflow-x-auto scrollbar-hide py-3 ml-2 space-x-2 menu-container"
          ...
          <div class="p-4 space-y-6">
            ...
            <div class="space-y-2">
              ...
              <select
                ...
                class="w-full text-sm border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              >
                <option value="HP_Native">Kamera HP</option>
...
// Di parent component (file induk)
const triggeredPoints = ref({});

// Handler untuk menerima data dari emit
const handleTriggerPointsChanged = (data) => {
  const { pointId, targetPointIds, action } = data;
  
  // Update triggeredPoints state
  triggeredPoints.value[pointId] = targetPointIds;
  
  // Jika ingin menghapus trigger ketika pilihan diubah
  if (action === 'update') {
    // Logic untuk mengelola triggered points
...
const isMenuComplete = (menu) => {
  if (menu.input_type === 'damage') {
    return true; // Selalu dianggap complete
  }
  ...
  const activeRequiredPoints = visiblePoints.filter(point => {
    return point.is_default && isPointCompatibleInMenu(point);
  });
  ...
  return activeRequiredPoints.every(point => {
    ...
    const settings = getPointSettings(point);
    const isRequired = settings.is_required !== false; // default true
