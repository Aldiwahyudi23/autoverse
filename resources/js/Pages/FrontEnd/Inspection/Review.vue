<!-- resources/js/Pages/FrontEnd/Inspection/Review.vue -->
<script setup>
...
import CardTransaction from '@/Components/Detail/CardTransaction.vue';
import AllDataModal from '@/Components/Detail/AllDataModal.vue';
...
// Computed untuk user role
const userRole = computed(() => {
  if (roles.includes('admin_plann')) return 'Admin AutoInspeksi';
  if (roles.includes('coordinator')) return 'Koordinator AutoInspeksi';
  if (roles.includes('inspector')) return 'Inspektor AutoInspeksi';
  if (roles.includes('quality_control')) return 'QC AutoInspeksi';
  return 'Staff AutoInspeksi';
});
...
const canShowPdf = computed(() => {
  return hasTransaction.value && isPaid.value && props.transaction?.amount > 0;
});
...
// Fungsi untuk membuka modal all data
...
// Fungsi setelah data disimpan
const handleSaved = () => {
  router.reload();
};

// Fungsi untuk mengirim email (masih dalam pengembangan)
const sendEmail = () => {
...
const getSeller = computed(() => {
  if (props.inspection.customer && props.inspection.customer.sellers) {
    return props.inspection.customer.sellers.find(seller => seller.inspection_id === props.inspection.id) || 
           props.inspection.customer.sellers[0] || 
           null;
  }
  return null;
});
...
<template>
  <AppLayout>
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
        ...
        <div class="px-4 py-3 bg-white border-t border-gray-100">
          <div class="flex justify-between items-center mb-3">
            ...
            <button 
              v-if="!hasAllData && !isStatus"
              @click="openAllDataModal"
          ...
          <!-- Komponen Customer Card -->
          <CardCustomer
            :customer="inspection.customer"
            :seller="getSeller"
            :show-edit-button="!isStatus && !hasAllData"
            :inspection="inspection"
            :user-role="userRole"
            @edit="openAllDataModal"
          />
        ...
        <div class="px-4 py-3 bg-gray-50 border-t border-gray-100">
          ...
          <div v-if="(inspection.status === 'approved' || inspection.status === 'completed') && inspection.file && canShowPdf" class="mt-3 mb-3">
            ...
            <p class="text-sm font-semibold text-gray-800 ml-7 -mt-1 flex items-center">
              ...
              <!-- Icon Clipboard -->
              <button @click="copyToClipboard(inspection.code)" class="ml-2 text-blue-500 hover:text-blue-700" title="Salin ID">
        ...
        <div class="p-4 flex gap-2">
          <!-- Tampilan tombol untuk status 'in_progress' -->
          ...
          <!-- Tampilan untuk status 'completed' -->
    ...
    <!-- Modal untuk semua data -->
    <AllDataModal
      v-if="showAllDataModal"
      :show="showAllDataModal"
      :inspection-id="encryptedIds"
    ...
    <div v-if="showEmailModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-6 rounded-lg shadow-xl w-96">
        ...
        <div class="flex justify-end space-x-4 mt-6">
          ...
          <button @click="sendEmail" :disabled="true" class="bg-teal-500 text-white px-4 py-2 rounded-md hover:bg-teal-600 opacity-50 cursor-not-allowed">
