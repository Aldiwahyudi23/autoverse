...
if (form.is_scheduled) {
    if (!form.scheduled_at_date || !form.scheduled_at_time) return false;
}
...
const handlePlateInput = (type) => {
    ...
    // combinePlateNumber();
    // Selalu update pesan validasi
    updateValidationMessage();
};

const updateValidationMessage = () => {
    // Jika kode wilayah ada dan valid
    if (plateAreaCode.value && plateCodes[plateAreaCode.value]) {
        inspectionValidationMessage.value = `✅ ${plateAreaCode.value} = ${plateCodes[plateAreaCode.value]}`;
        isPlateInvalid.value = false;
    } 
    // Jika kode wilayah ada tapi tidak valid
    else if (plateAreaCode.value && !plateCodes[plateAreaCode.value]) {
        inspectionValidationMessage.value = `🚫 Kode wilayah "${plateAreaCode.value}" tidak valid`;
        isPlateInvalid.value = true;
    } 
    // Jika kode wilayah kosong
    else {
        inspectionValidationMessage.value = '⚠ Masukkan kode wilayah (contoh: D, B, DK)';
        isPlateInvalid.value = true;
    }
};
