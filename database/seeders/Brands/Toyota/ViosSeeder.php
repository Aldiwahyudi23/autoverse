...
class ViosSeeder extends Seeder
{
    ...
    private function generateDescription($typeName, $year, $cc, $engineCode, $transmission, $fuelType, $generation, $power): string
    {
        ...
        $specialFeatures = " Dikenal sebagai sedan andalan dengan reputasi kuat di pasar Indonesia.";
        if ($typeName === 'GR Sport') {
            $specialFeatures = " Varian sporty dengan sentuhan Gazoo Racing yang agresif.";
        }

        return "Toyota Vios {$typeName} {$ccText} {$engineCode} {$transmissionText} {$fuelText} tahun {$year}. " .
               "Generasi {$generation} dengan tenaga {$power}. Kendaraan {$bodyType} yang populer karena efisiensi bahan bakar dan keandalannya.{$specialFeatures}";
    }
